...
ProcMon.prototype = {
  ...
  /**
   * Handle the specified event only once - exposed for testing.
   * @param  {string}   event    The name of the event.
   * @param  {Function} callback The function callback.
   */
  ...
  updateConnections: function() {
    ...
    exec('netstat -a | grep \'LISTEN\' | awk \'{print $4}\'', function(err, stdout, stderr) {
      if (err) {
        self.events.emit('error', err);
      } else {
        var connections = stdout.split('\n');
        self.events.emit('connections', { count: connections.length, connections: connections });
      }
    });
  }
};
